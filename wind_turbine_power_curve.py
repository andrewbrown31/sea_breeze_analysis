...
def capacity_factor(ws, Pnom=5700, D=163, Vcutin=3, Vcutoff=25):


    """
    Added by Andrew Brown, 2025

    Calculate the capacity factor of a wind turbine using a logistic function fitted to the GenericWindTurbinePowerCurve.
    GenericWindTurbinePowerCurve is a parametric model for wind turbine power curves incorporating environmental conditions.

    Logistic function idea from: https://iopscience.iop.org/article/10.1088/1748-9326/ab91e9

    Parameters
    ----------
    ws : xarray dataarray
        Wind speeds at which to calculate the capacity factor.
    Pnom : float, optional
        Nominal power of the wind turbine in watts. Default is 5700 W (MacIntyre Wind Farm, Nodex N163/5.7).
    D : float, optional
        Rotor diameter in meters. Default is 163 m.
    Vcutin : float, optional
        Cut-in wind speed in m/s. Default is 3 m/s.
    Vcutoff : float, optional
        Cut-off wind speed in m/s. Default is 25 m/s.
    """

    import scipy
    import xarray as xr

    def logistic_fn(x,A,C,T,B,M):
        return A + C / np.power((1 + T*np.exp(-B * (x - M))),1/T)

    #Create a list of wind speeds from cut in to cut out speed
    x = np.linspace(3,25,1000)

    #Calculate the wind power capacity factor using the GenericWindTurbinePowerCurve function, without any cut in or cut off speeds
    generic_curve = GenericWindTurbinePowerCurve(x,Pnom,D,Vcutoff=np.inf,Vcutin=3)/Pnom

    #Fit the logistic function to the generic curve
    p,_ = scipy.optimize.curve_fit(logistic_fn, x, generic_curve, p0=[0,1,1,1,8])
    A, C, T, B, M = p

    #Now for an array of wind speeds, calculate the logistic function
    cf = logistic_fn(ws, A, C, T, B, M)

    #Add back the cut-in and cut-off wind speeds and set cf lower limit to 0
    cf = xr.where(cf < 0, 0, cf)  # Ensure no negative values
    cf = xr.where(ws < Vcutin, 0, cf)  # Set capacity factor to 0 below cut-in speed
