...
#PBS -P ng72 
#PBS -q hugemem
#PBS -l walltime=24:00:00,mem=760GB 
#PBS -l ncpus=96
#PBS -l jobfs=32gb
...
no_wind_threshold="0.2564932795026744"	
no_q_threshold="0.21298706399406103"
no_t_threshold="0.18887677233183595"
...
while [[ "$current_date" < "$end_date" ]]; do
    ...
    if [[ "$current_date" == *"-01-"* || "$current_date" == *"-02-"* ]]; then
        if [[ "$current_date" == *"2013"* || "$current_date" == *"2016"* || "$current_date" == *"2018"* ]]; then
            ...
            #Fuzzy with no wind change
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name no_hourly_change_v2 --field_name fuzzy_no_wind --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $no_wind_threshold --exp_id $exp_id

            #Fuzzy with no q change
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name no_hourly_change_v2 --field_name fuzzy_no_q --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $no_q_threshold --exp_id $exp_id

            #Fuzzy with no t change
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name no_hourly_change_v2 --field_name fuzzy_no_t --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $no_t_threshold --exp_id $exp_id
