...
#PBS -l ncpus=48
#PBS -l jobfs=32gb
#PBS -o /home/548/ab4502/working/ExtremeWind/jobs/messages/aus2200_smooth_s4_filter_orientation2.o 
#PBS -e /home/548/ab4502/working/ExtremeWind/jobs/messages/aus2200_smooth_s4_filter_orientation2.e
...
while [[ "$current_date" < "$end_date" ]]; do
    ...
    if [[ "$current_date" == *"-01-"* || "$current_date" == *"-02-"* ]]; then
        if [[ "$current_date" == *"2013"* || "$current_date" == *"2016"* || "$current_date" == *"2018"* ]]; then
            ...
            #F
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name orientation2 --field_name F --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $f_threshold --exp_id $exp_id --no-orientation_filter

            #Fuzzy
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name orientation2 --field_name fuzzy --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $fuzzy_threshold --exp_id $exp_id --no-orientation_filter

            #sbi
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s4 --filter_name orientation2 --field_name sbi --t1 "$sbi_start_time" --t2 "$end_time" --threshold fixed --threshold_value $sbi_threshold --exp_id $exp_id --no-orientation_filter
