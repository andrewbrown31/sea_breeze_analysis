...
f_threshold="51.76884435116627"	
fuzzy_threshold="0.2720985539345352"
sbi_threshold="0.37456651772536514"
...
while [[ "$current_date" < "$end_date" ]]; do
    ...
    if [[ "$current_date" == *"-01-"* || "$current_date" == *"-02-"* ]]; then
        if [[ "$current_date" == *"2013"* || "$current_date" == *"2016"* || "$current_date" == *"2018"* ]]; then
            ...
            #F
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s2 --filter_name smooth1 --field_name F --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $f_threshold --exp_id $exp_id

            #Fuzzy
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s2 --filter_name smooth1 --field_name fuzzy --t1 "$start_time" --t2 "$end_time" --threshold fixed --threshold_value $fuzzy_threshold --exp_id $exp_id

            #sbi
            python /home/548/ab4502/working/sea_breeze/filter.py --model aus2200_smooth_s2 --filter_name smooth1 --field_name sbi --t1 "$sbi_start_time" --t2 "$end_time" --threshold fixed --threshold_value $sbi_threshold --exp_id $exp_id
