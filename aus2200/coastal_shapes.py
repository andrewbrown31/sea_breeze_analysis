...
if __name__ == "__main__":
    ...
    rez_mask = xr.open_dataset("/g/data/ng72/ri9247/data/masks/rez_mask_AUST-04.nc")

    #Load AUS2200 static info
    angle_ds = xr.open_dataset(
        "/g/data/ng72/ab4502/coastline_data/aus2200.nc"
    )
    lsm = xr.open_dataset(
        "/g/data/bs94/AUS2200/mjo-elnino2016/v1-0/fx/lmask/lmask_AUS2200_mjo-elnino_fx.nc"
    ...
    gipps_shape,gipps_shape_landsea = define_coastal_region(gipps_lat_slice,gipps_lon_slice,angle_ds,lsm.lmask==100)

    #Illawara
    illawara_lat_slice = slice(-35,-33.8)
    illawara_lon_slice = slice(149,154)
    illawara_shape,illawara_shape_landsea = define_coastal_region(illawara_lat_slice,illawara_lon_slice,angle_ds,lsm.lmask==100)

    #Newcastle
    newcastle_lat_slice = slice(-33.5,-32)
    newcastle_lon_slice = slice(149,154)
    newcastle_shape,newcastle_shape_landsea = define_coastal_region(newcastle_lat_slice,newcastle_lon_slice,angle_ds,lsm.lmask==100)

    #Southern Ocean
    southern_lat_slice = slice(-39.8,-37)
    southern_lon_slice = slice(141,143.5)
    southern_shape,southern_shape_landsea = define_coastal_region(southern_lat_slice,southern_lon_slice,angle_ds,lsm.lmask==100)

    #Tasmania
    tas_lat_slice = slice(-41.5,-40.5)
    tas_lon_slice = slice(145.8,147.8)
    tas_shape,tas_shape_landsea = define_coastal_region(tas_lat_slice,tas_lon_slice,angle_ds,lsm.lmask==100)

    #South Australia
    sa_lat_slice = slice(-38.2,-36)
    sa_lon_slice = slice(138.8,140.5)
    sa_shape,sa_shape_landsea = define_coastal_region(sa_lat_slice,sa_lon_slice,angle_ds,lsm.lmask==100)

    #Bunbury
    bunbury_lat_slice = slice(-33.8,-31.9)
    bunbury_lon_slice = slice(115.2,116.3)
    bunbury_shape,bunbury_shape_landsea = define_coastal_region(bunbury_lat_slice,bunbury_lon_slice,angle_ds,lsm.lmask==100)    
    ...
    shapes.to_netcdf("/g/data/ng72/ab4502/coastline_data/rez_coastal_shapes_aus2200.nc")