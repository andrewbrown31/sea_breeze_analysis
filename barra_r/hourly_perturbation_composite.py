...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="BARRA-R hourly perturbation composite",
        description="This program calculates the average hourly perturbation of the rotated wind components from a rolling daily mean for BARRA-R data."
    )
    ...
    hourly = xr.Dataset(
         {"ws":ws.groupby("time.hour").mean(),
         "cf":cf.groupby("time.hour").mean()})
    #hourly = xr.Dataset({"cf":cf.groupby("time.hour").mean()})        
    ...
    #encoding = {var: comp for var in hourly.data_vars}

    output_path = "/scratch/ng72/ab4502/hourly_composites/barra_r/barra_r_hourly_"+\
        u_var+"_"+v_var+"_"+\
